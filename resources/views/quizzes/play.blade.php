    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        @php
            ...
            $initialState = [
                ...
                'player_answer' => $currentAnswer ? [
                    'question_option_id' => $currentAnswer->question_option_id,
                    'answer_text' => $currentAnswer->answer_text,
                    'answer_numeric' => $currentAnswer->answer_numeric,
                    'answer_date' => $currentAnswer->answer_date?->format('Y-m-d'),
                    'answer_json' => $currentAnswer->answer_json,
                    'is_correct' => (bool) $currentAnswer->is_correct,
                ] : null,
                'question' => $currentQuestion ? [
                    ...
                    'correct_numeric_answer' => $isRevealed ? $currentQuestion->correct_numeric_answer : null,
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <span id="quiz-progress-badge" class="badge badge-secondary">
                    Frage {{ $currentQuestion ? ($currentIndex + 1) : 0 }}/{{ $totalQuestions }}
                </span>
            </div>
        </div>
        ...
        <div id="quiz-status-alerts" class="space-y-3">
            @if($quiz->status === 'scheduled')
                <div class="alert alert-warning">
                    <span>
                        Dieses Quiz ist geplant und noch nicht gestartet.
                        @if($quiz->starts_at)
                            Geplanter Start: {{ $quiz->starts_at->format('d.m.Y H:i') }}
                        @endif
                    </span>
                </div>
            @endif
        ...
        <div id="question-card" class="card bg-base-100 shadow-xl border border-base-300 {{ $isPaused ? 'hidden' : '' }}">
            <div class="card-body">
                ...
                <div id="question-empty" class="{{ $currentQuestion ? 'hidden' : '' }} text-base-content/60">
                    Aktuell ist keine Frage aktiv.
                </div>

                <div id="question-content" class="{{ $currentQuestion ? '' : 'hidden' }} space-y-5">
                    ...
                    <div id="revealed-alert" class="hidden"></div>
                </div>
            </div>
        </div>
...
@vite('resources/js/quiz-player.js')
@endsection